...
import com.tresbu.demo.core.managers.ServerManager;
...
public class ADB {
	...
	public static String command(String command){
		DemoLogger.log.debug("Formatting ADB Command : "+command);
		...
		DemoLogger.log.debug("Formatted ADB Command : "+command);
		String output = ServerManager.runCommand(command);
		DemoLogger.log.debug("Output of ADB Command : "+output);
		if(output == null || output == "" || output == " ") return "";
	...
	public void openAppsActivity(String packageID,String activityID){
		command("adb -s "+ID+" shell am start -c api.android.intent.LAUNCHER -a api.android.category.MAIN -n "+packageID+"/"+activityID);
	}
	...
	public Object startLogcat(final String logID, final String grep){
		...
		else if(pidAfter.size() > 1) throw new RuntimeException("Multiple logcat processess is started when only one was expected!");
